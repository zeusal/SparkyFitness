...
export const DetectPatternsSchema = DaysRangeSchema.extend({
  days: z.coerce.number().int().min(1).max(90).default(30).describe("Number of days to analyze for patterns (1-90)"),
}).strict();

export const GenerateCoachingPlanSchema = z.object({
  goal: z.enum(["weight_loss", "muscle_gain", "maintenance"]).default("maintenance"),
...
export type GenerateCoachingPlanInput = z.infer<typeof GenerateCoachingPlanSchema>;
