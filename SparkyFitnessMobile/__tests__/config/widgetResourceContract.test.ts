...
describe('Android widget localization contract', () => {
  describe('string resources', () => {
    ...
    it('keeps the two widget names distinct in every shipped locale', () => {
      for (const locale of SHIPPED_WIDGET_LOCALES) {
        const strings = new Map(readWidgetStringResources(locale).map((r) => [r.name, r.value]));
        expect(strings.get('sparky_calorie_widget_name')).not.toBe(strings.get('sparky_macro_widget_name'));
      }
    });
