...
describe('unregistered locale catalogs', () => {
  // The mobile sync copies every Weblate language, so a translator error in one
  // nobody ships must not redden CI; registered locales stay blocking.
  const BROKEN = JSON.stringify({ greeting: 'Hola {{wrong}}' });
  const SOURCE = JSON.stringify({ greeting: 'Hello {{name}}' });

  /** Drop a locale from the fixture registry while leaving its directory on disk. */
