...
function validateSurface({ maps, source, surface, formatRegex }) {
  ...
  for (const [locale, target] of maps) {
    ...
    for (const [key, value] of target) {
      if (!base.has(key)) { errors.push(`${surface} ${locale}:${key} does not exist in source`); continue; }
      if (isBlank(value)) { continue; }
      if (/\{\{.*?\}\}/.test(value)) errors.push(`${surface} ${locale}:${key} uses i18next placeholder syntax`);
      if (!equal(formats(base.get(key), formatRegex), formats(value, formatRegex))) errors.push(`${surface} ${locale}:${key} placeholder mismatch`);
      translated += 1;
    }
...
export function validateNativeWidgetLocales({ root, registry }) {
  ...
  for (const entry of fs.readdirSync(androidRoot, { withFileTypes: true })) {
    ...
    if (locale && fs.existsSync(file)) android.set(locale, parseAndroidStrings(fs.readFileSync(file, 'utf8')));
  }
  const ios = new Map();
  for (const entry of fs.readdirSync(iosRoot, { withFileTypes: true })) {
    ...
    if (fs.existsSync(file)) ios.set(locale, parseIosStrings(fs.readFileSync(file, 'utf8')));
  }
  const androidResult = validateSurface({ maps: android, source, surface: 'Android widget', formatRegex: ANDROID_FORMAT });
  const iosResult = validateSurface({ maps: ios, source, surface: 'iOS widget', formatRegex: IOS_FORMAT });
  ...
  return { errors, androidCoverage: androidResult.coverage, iosCoverage: iosResult.coverage, androidDirForLocale };
}
