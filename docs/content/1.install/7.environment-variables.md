...
- **`SPARKY_FITNESS_DB_HOST`**: The hostname of the PostgreSQL database. For Docker Compose, this defaults to `sparkyfitness-db` (the service name). Only change this if you are using an external database server. A value starting with `/` (e.g. `/var/run/postgresql`) is treated as a Unix socket directory instead of a hostname — see [External Database Setup](/install/external-database).
...
### Client IP Detection (Server Side)

Only relevant when a proxy, tunnel, or CDN sits in front of SparkyFitness. These decide which address the server treats as the visitor's, which drives per-IP rate limiting and audit logs. Get it wrong and every visitor appears as the same internal address, so a per-IP limit throttles all of them together as a single caller.

- **`SPARKY_FITNESS_TRUSTED_PROXY_HOPS`**: Number of proxies in front of the app. Defaults to `1`, which matches the bundled setup where only the frontend container's Nginx is in front. Count every additional hop — for example `Cloudflare Tunnel -> Nginx Proxy Manager -> frontend` is `3`.
- **`SPARKY_FITNESS_REAL_IP_HEADER`**: Name of a header that a CDN stamps the visitor's address into, used instead of counting hops. Sturdier than the hop count, because it keeps working when you add or remove a proxy. Use `CF-Connecting-IP` for Cloudflare, or `True-Client-IP` for Akamai and several others. Unset by default.

  Set this only when the server cannot be reached except through that CDN. If it is reachable directly, anyone can send the header themselves and bypass the rate limits. Being reachable from your own LAN is usually acceptable; forwarding a port to it on your router is not.
...
- **`SPARKY_FITNESS_DEMO_MODE`**: Set to `true` to enable Demo Mode. When enabled:
  ...
  - Protects the demo account from modification. A single deny-list runs ahead of the whole route table and blocks, for the demo account only: all account management under `/api/identity` (password, email, 2FA/TOTP, passkeys, API keys, family sharing, profile), the equivalent Better Auth endpoints, every file upload on any route (including CSV/JSON/FIT imports and base64 photo analysis, which do not use multipart), publishing to Open Food Facts, AI features that bill the operator's LLM provider (`/api/chat`, `/api/ai`, `/mcp`), third-party integration binding (`/api/integrations`, `/api/withings`, `/api/external-providers`), and the admin surface.
